...
public class OperacionesCRUD {
    ...
    public List<Oficina> listarOficinas() { return oficinas; }

    public boolean borrarOficina(int id) {
        return oficinas.removeIf(o -> o.getNumeroOficina() == id);
    }

    public Oficina buscarOficina(int id) {
        return oficinas.stream()
                .filter(o -> o.getNumeroOficina() == id)
                .findFirst()
                .orElse(null);
    }
    ...
    public List<Vendedor> listarVendedores() { return vendedores; }

    public boolean borrarVendedor(int id) {
        return vendedores.removeIf(v -> v.getNumeroEmpleado() == id);
    }

    public Vendedor buscarVendedor(int id) {
        return vendedores.stream()
                .filter(v -> v.getNumeroEmpleado() == id)
                .findFirst()
                .orElse(null);
    }
    ...
    public boolean borrarCliente(int id) {
        return clientes.removeIf(c -> c.getNumeroCliente() == id);
    }
    ...
    public boolean borrarProducto(String cod) {
        return productos.removeIf(p -> p.getCodigoProducto().equalsIgnoreCase(cod));
    }

    public Producto buscarProducto(String cod) {
        return productos.stream()
                .filter(p -> p.getCodigoProducto().equalsIgnoreCase(cod))
                .findFirst()
                .orElse(null);
    }
    ...
    public boolean borrarPedido(int id) {
        return pedidos.removeIf(p -> p.getNumeroPedido() == id);
    }

    public Pedido buscarPedido(int id) {
        return pedidos.stream()
                .filter(p -> p.getNumeroPedido() == id)
                .findFirst()
                .orElse(null);
    }
    ...
    public String MostrarOficinasVendedoresClientes() {
        ...
        for (Oficina o : oficinas) {
            sb.append("Oficina " + o.getNumeroOficina() + " - " + o.getCiudad() + "\n");

            for(Vendedor v: vendedores) {
                if (v.getNumeroOficina() == o.getNumeroOficina()) {
                    sb.append("\tVendedor: " + v.getNombre() + "\n");

                    for (Cliente c: clientes) {
                        if (c.getNumeroEmpleado() == v.getNumeroEmpleado()) {
                            sb.append("\t\tCliente: " + c.getEmpresa() + "\n");
                        }
                    }
                }
            }
    ...
    public Vendedor MostrarVendedorConMasVentas() {
        ...
        max = vendedores.stream()
                .max(Comparator.comparingDouble(Vendedor::getVentas)).get();
    ...
    public Cliente MostrarClienteconMayorCredito() {
        ...
        max = clientes.stream()
                .max(Comparator.comparingDouble(Cliente::getLimiteCredito)).get();
    ...
    public Producto MostrarProductoconMenorStock() {
        ...
        min = productos.stream()
                .min(Comparator.comparingInt(Producto::getExistencias)).get();
